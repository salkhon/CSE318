...
std::shared_ptr<Board> BoardFactory::move_board(std::shared_ptr<Board> prev_board, Move move) {
    auto [row, col] = get_blank_idx(prev_board->board);
    std::shared_ptr<Board> board_ptr;
    std::vector<std::vector<int>> board{ prev_board->board };
    ...
    if (move == LEFT && col > 0) {
        ...
        board_ptr = std::make_shared<Board>(nullptr);
    }

    return board_ptr;
}

std::shared_ptr<Board> BoardFactory::init_board(const std::vector<std::vector<int>>& board) {
    return std::make_shared<Board>(board, 0, nullptr);
}
